...
defineTypeNameAndDebug(DALaunderSharmaKE, 0);
addToRunTimeSelectionTable(DATurbulenceModel, DALaunderSharmaKE, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DALaunderSharmaKE::DALaunderSharmaKE(
...
// KE member functions
tmp<fvScalarMatrix> DALaunderSharmaKE::kSource() const
{
...
tmp<fvScalarMatrix> DALaunderSharmaKE::epsilonSource() const
{
    return tmp<fvScalarMatrix>(
        new fvScalarMatrix(
            epsilon_,
...
tmp<volScalarField> DALaunderSharmaKE::DkEff() const
{
...
tmp<volScalarField> DALaunderSharmaKE::DepsilonEff() const
{
    return tmp<volScalarField>(
        new volScalarField(
            "DepsilonEff",
...
// Launder-Sharma's modification
tmp<volScalarField> DALaunderSharmaKE::fMu() const
{
    return exp(-3.4/sqr(scalar(1) + sqr(k_)/(this->nu()*epsilon_)/50.0));
}

tmp<volScalarField> DALaunderSharmaKE::f2() const
{
    return
        scalar(1)
...
// Augmented functions
void DALaunderSharmaKE::correctModelStates(wordList& modelStates) const
{
    /*
    Description:
...
void DALaunderSharmaKE::correctNut()
{
    /*
    Description:
...
void DALaunderSharmaKE::correctBoundaryConditions()
{
    /*
    Description:
...
void DALaunderSharmaKE::correctEpsilonBoundaryConditions()
{
    /*
    Description:
...
void DALaunderSharmaKE::saveEpsilonNearWall()
{
    /*
    Description:
...
void DALaunderSharmaKE::setEpsilonNearWall()
{
    /*
    Description:
...
void DALaunderSharmaKE::updateIntermediateVariables()
{
    /*
    Description:
...
void DALaunderSharmaKE::correctStateResidualModelCon(List<List<word>>& stateCon) const
{
    /*
    Description:
...
void DALaunderSharmaKE::addModelResidualCon(HashTable<List<List<word>>>& allCon) const
{
    /*
    Description:
...
void DALaunderSharmaKE::correct()
{
    /*
    Descroption:
...
void DALaunderSharmaKE::calcResiduals(const dictionary& options)
{
    /*
    Descroption:
    ...
    */
    ...
    // load alphaPorosity terms and the omegab
    const volScalarField& alphaPorosity_ = mesh_.thisDb().lookupObject<volScalarField>("alphaPorosity");
    const volScalarField& alphaMax_ = mesh_.thisDb().lookupObject<volScalarField>("alphaMax");
    const volScalarField& q1_ = mesh_.thisDb().lookupObject<volScalarField>("q1");
    const volScalarField& q2_ = mesh_.thisDb().lookupObject<volScalarField>("q2");
    const volScalarField& turFac_ = mesh_.thisDb().lookupObject<volScalarField>("turFac");
    ...
    // Launder-Sharma's modification
    volScalarField E(2.0*this->nu()*nut_*fvc::magSqrGradGrad(U_));
    volScalarField D(2.0*this->nu()*magSqr(fvc::grad(sqrt(k_))));
    volScalarField ff2 = f2();
    volScalarField& ff2_ = ff2;
    ...
    tmp<fvScalarMatrix> epsEqn(
        ...
        == C1_ * phase_() * rho_() * G * epsilon_() / k_()
            ...
            - fvm::Sp(C2_* phase_() * rho_() * ff2_  * epsilon_() / k_(), epsilon_)
            + phase_() * rho_() * E
            + epsilonSource());
    ...
    tmp<fvScalarMatrix> kEqn(
        ...
        == phase_() * rho_() * G
            ...
            - fvm::Sp(phase_() * rho_() * (epsilon_() + D) / k_(), k_)
            - fvm::Sp(volScalarField("penaltyk", turFac_ * q1_ * alphaPorosity_ / (q2_ - alphaPorosity_ / alphaMax_ )), k_)
            + kSource());

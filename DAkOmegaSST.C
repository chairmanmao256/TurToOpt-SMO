...
{
    // use p-poisson distance, Wcy, 2022
...
{
    const volScalarField& d_ = mesh_.thisDb().lookupObject<volScalarField>("DWall");
    ...
    tmp<volScalarField> arg1 = min(
        min(
            max(
                (scalar(1) / betaStar_) * sqrt(k_) / (omega_ * d_),
                scalar(500) * (this->nu()) / (sqr(d_) * omega_)),
            (scalar(4) * alphaOmega2_) * k_ / (CDkOmegaPlus * sqr(d_))),
        scalar(10));
...
tmp<volScalarField> DAkOmegaSST::F2() const
{
    ...
    tmp<volScalarField> arg2 = min(
        max(
            (scalar(2) / betaStar_) * sqrt(k_) / (omega_ * d_),
            scalar(500) * (this->nu()) / (sqr(d_) * omega_)),
        scalar(100));
...
void DAkOmegaSST::calcResiduals(const dictionary& options)
{
    ...
    */
    ...
    // Info<<"Loading alphaPorosity and related terms in the turbulence model.\n";
    ...
    const volScalarField& omegab_ = mesh_.thisDb().lookupObject<volScalarField>("omegab");
...

    {
        ...
        tmp<fvScalarMatrix> omegaEqn(
            ...
            == phase_() * rho_() * gamma * GbyNu(GbyNu0, F23(), S2())
                ...
                + Qsas(S2(), gamma, beta)
                + omegaSource()
                + fvc::SuSp(volScalarField("penaltyw1", turFac_ * q1_ * alphaPorosity_ / (q2_ - alphaPorosity_ / alphaMax_)), omegab_)
                - fvm::Sp(volScalarField("penaltyw2", turFac_ * q1_ * alphaPorosity_ / (q2_ - alphaPorosity_ / alphaMax_)), omega_)
